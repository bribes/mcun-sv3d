...
import { Color, ColorRepresentation, PointLight, EquirectangularReflectionMapping, Group, NearestFilter, PerspectiveCamera, Scene, Texture, Vector2, WebGLRenderer, AmbientLight, Mapping, CanvasTexture, WebGLRenderTarget, FloatType, DepthTexture, Object3D } from "three";
...
export interface SkinViewerOptions {
	...
	/**
	 * Shifts the user. Default is false.
	 */
		 shift?: boolean
...
export class SkinViewer {
	...
	constructor(options: SkinViewerOptions = {}) {
		...
		this.playerObject = new PlayerObject(options.shift == true);
		this.playerObject.name = "player";
		this.shifting = false;
		if (options.shift == true) {
			this.shifting = true;
			this.playerObject.shift();
		} else {
			if (this.shifting = true) { this.playerObject.unshift(); }
			this.shifting = false;
		}
