...
export const WalkingAnimation: Animation = (player, time) => {
	...
	if (!player.shifting) {
		...
		// Arm swing
		skin.leftArm.rotation.x = Math.sin(time + Math.PI) * 0.25;
		skin.rightArm.rotation.x = Math.sin(time) * 0.25;
		// Leg swing
		skin.leftLeg.rotation.x = Math.sin(time) * 0.15;
		skin.rightLeg.rotation.x = Math.sin(time + Math.PI) * 0.15;

		// Always add an angle for cape around the x axis
		const basicCapeRotationX = Math.PI * 0.23;
		player.cape.rotation.x = Math.sin(time / 1.5) * 0.05 + basicCapeRotationX;
	}
};
